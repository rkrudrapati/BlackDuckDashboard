...
dnd =[]
with open('dont_delete.txt', 'r') as dont_delete:
    for lines in dont_delete.readlines():
        dnd.append(lines.strip())
    print(dnd)
    dont_delete.close()
...
for items in all_project_items:
    ...
    if project_name == variables.key_project_name:
        print("Deleting empty version in project: %s" % project_name)
        ...
        for v_items in all_version_items:
            ...
            codelocation_link = bd_utils.identify_href_link(v_items,'codelocations')
            each_version_response = get_request(url=codelocation_link, headers=get_headers)
            codelocation_count = bd_utils.get_codelocation_count(each_version_response)
            # if codelocation_count == 0:
            #     # print(project_name)
            #     print(version_name)
            #     version_id = version_link.split('/')[-1]
            #     # bd_utils.delete_version(project_id, version_id)
            #     bd_utils.delete_version_new(version_id)
            if version_name not in dnd:
                print(version_name)
                version_id = version_link.split('/')[-1]
                bd_utils.delete_version_new(version_id)
